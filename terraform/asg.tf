...
resource "aws_autoscaling_policy" "scale_out" {
  name                   = "${local.prefix}-ecs-scale-out"
...
resource "aws_cloudwatch_metric_alarm" "cpu_high" {
  ...
  dimensions          = { AutoScalingGroupName = aws_autoscaling_group.ecs.name }
}

# ── Scale-in policy: remove instances when CPU < 30% ──────────────────────
resource "aws_autoscaling_policy" "scale_in" {
  name                   = "${local.prefix}-ecs-scale-in"
  autoscaling_group_name = aws_autoscaling_group.ecs.name
  adjustment_type        = "ChangeInCapacity"
  scaling_adjustment     = -1
  cooldown               = 300
...
resource "aws_cloudwatch_metric_alarm" "cpu_low" {
  alarm_name          = "${local.prefix}-ecs-cpu-low"
